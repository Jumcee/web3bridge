...
contract PrizeDistribution is Ownable, VRFConsumerBase {
    ...
    // Function for participants to earn entries based on activity
    function earnEntries(uint256 _activityLevel) external {
        // Implement logic to calculate entries based on activity
        uint256 entriesEarned = calculateEntries(_activityLevel);

        // Add entries to the participant
        participants[participantIndex[msg.sender] - 1].entries += entriesEarned;

        emit EntriesEarned(msg.sender, entriesEarned);
    }

    // Function to calculate entries based on activity
    function calculateEntries(uint256 _activityLevel) internal pure returns (uint256) {
        return _activityLevel * 2; // Example: Entries = 2 * Activity Level
    }
    ...
    // function to set the distribution start time
    function setDistributionStartTime(uint256 _startTime) external onlyOwner {
        distributionStartTime = _startTime;
    }

    // function to set the minimum entries required for distribution
